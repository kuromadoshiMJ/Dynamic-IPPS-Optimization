...
class Chromosome:
    def __init__(self, num_jobs, len_plan_max, num_operations_arr, bd_start = None, bd_end = None, bd_machine = None) -> None:
        ...
        for i in range(num_jobs):
            self.operation_fixed_gene.append(np.zeros(num_operations_arr[i], dtype=int))

    def set_fault_status(self, bd_start = None, bd_end = None, bd_machine = None):
        self.bd_start = bd_start
        self.bd_end = bd_end
        self.bd_machine = bd_machine
        if bd_machine is not None: 
            ...
            self.machine_engage_time_dict = {}
    def encode_chromosome_abd(self, num_jobs, num_process_plan_arr, len_process_plan_arr, num_operations_arr, len_plan_max, alt_machines_dict) -> None:
        for i in range(num_jobs):
            job_id = i+1
            self.generate_machine_gene_abd(num_operations_arr, job_id, alt_machines_dict)
            self.generate_tool_gene_abd(num_operations_arr, job_id)

    def generate_machine_gene_abd(self, num_operations_arr, job_id, alt_machines_dict):
        
        operation_machine_dict = alt_machines_dict[job_id]
        
        for i in range(num_operations_arr[job_id -1]):
            if self.operation_fixed_gene[job_id-1][i] == 0: 
                self.operation_machine_gene[job_id-1][i] = np.random.choice(operation_machine_dict[i+1])

    def generate_tool_gene_abd(self, num_operations_arr, job_id):

        for i in range(num_operations_arr[job_id-1]):
            if self.operation_fixed_gene[job_id-1][i] == 0:
                self.operation_tool_gene[job_id-1][i] = np.random.choice([0, 1, 2])
    ...
    def decode_fixed_subgene_abd(self, num_jobs, num_machines, len_plan_max, process_plan_matrix, process_time_matrix, energy_matrix):
        freq_dict = {}
        self.machine_engage_time_dict = {}
        ...
        for i in self.schedule_plan_gene:
            ...
            if completion_time[i-1][freq_dict[i]-1]<=self.bd_start:
                self.operation_fixed_gene[i-1][operation_id-1] = 1
    ...
    def decode_chromosome(self, num_jobs, num_machines, len_plan_max, process_plan_matrix, process_time_matrix, energy_matrix):
        ...
        for i in self.schedule_plan_gene:
            ...
            if(self.bd_start is not None) and (self.operation_fixed_gene[i-1][operation_id-1] == 0):
                allowable_start_time[i-1][freq_dict[i]-1] = max(allowable_start_time[i-1][freq_dict[i]-1], self.bd_start)
            # print("len", len(self.operation_tool_gene))
            # for j in self.operation_tool_gene:
            #     print(j)
    ...
    def plot_gantt_chart(self):
        ...
        for index, row in df.iterrows():
            ...
            # Add black marks for the start and end of each operation
            ax.plot([row['Start'], row['Start']], [row['Machine ID'] - 0.4, row['Machine ID'] + 0.4], color='black', linewidth=1)  # Start mark
            ax.plot([row['End'], row['End']], [row['Machine ID'] - 0.4, row['Machine ID'] + 0.4], color='black', linewidth=1)  # End mark
        ...
        x_min = 0  # Minimum start time
        x_max = 100    # Maximum end time
        ax.set_xlim(x_min, x_max)

        # Define the number of ticks and their positions
        num_ticks = 11  # You can change this to adjust the number of ticks
        tick_positions = np.linspace(x_min, x_max, num_ticks)
        ax.set_xticks(tick_positions)

        # Add a legend for job colors
        legend_patches = [mpatches.Patch(color=color, label=f'Job ID {job_id}') for job_id, color in colo_mapping.items()]
        ax.legend(handles=legend_patches, title="Job ID Colors")
